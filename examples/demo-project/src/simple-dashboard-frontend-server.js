...
class SimpleDashboardFrontendServer {
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            ...
            line-height: 1.6; font-size: ${fontSize}px; transition: all 0.3s ease;
        }
        .container { display: flex; min-height: 100vh; transition: all 0.3s ease; }
        .sidebar {
            width: ${showSidebar ? sidebarWidth : 0}px; background: rgba(0, 0, 0, 0.2);
            backdrop-filter: blur(10px); padding: 20px; transition: all 0.3s ease;
            overflow: hidden; ${showSidebar ? '' : 'display: none;'}
        }
      ...
      try {
        ...
        this.io.on('connection', (socket) => {
          ...
          socket.emit('dashboard-update', {
            message: 'Connected to Simple Dashboard-Frontend Integration System',
            timestamp: new Date().toISOString()
          });
        ...
        this.server.listen(this.port, () => {
          this.isRunning = true;
          console.log(`🖖 Alex AI Simple Dashboard-Frontend Integration Server running on http://localhost:${this.port}`);
  ...
  async stop() {
    return new Promise((resolve) => {
      if (this.server && this.isRunning) {
        this.server.close(() => {
          this.isRunning = false;
          console.log(`🛑 Alex AI Simple Dashboard-Frontend Integration Server stopped on port ${this.port}`);
          resolve();
        });
      } else {
        resolve();
...
if (require.main === module) {
  const server = new SimpleDashboardFrontendServer();
  ...
  // Graceful shutdown
  process.on('SIGTERM', () => {
    console.log('SIGTERM signal received. Shutting down simple dashboard-frontend integration server...');
    server.stop().then(() => process.exit(0));
  });

  process.on('SIGINT', () => {
    console.log('SIGINT signal received. Shutting down simple dashboard-frontend integration server...');
    server.stop().then(() => process.exit(0));
  });
}
