...
class DashboardFrontendIntegrationServer {
  constructor(port = 3000) {
    this.port = port;
    this.server = null;
    this.io = null;
    this.isRunning = false;
    this.currentView = 'dashboard'; // 'dashboard' or 'live'
    ...
    const crewData = [
      {
        id: 'picard',
        name: 'Jean-Luc',
        fullName: 'Captain Jean-Luc Picard',
      ...
      },
      {
        id: 'riker',
        name: 'William',
        fullName: 'Commander William Riker',
        role: 'First Officer',
      ...
      },
      {
        id: 'laforge',
        name: 'Commander',
        fullName: 'Lieutenant Commander Geordi La Forge',
        role: 'Chief Engineer',
      ...
      },
      {
        id: 'troi',
        name: 'Deanna',
        fullName: 'Counselor Deanna Troi',
...
<head>
    ...
    <title>🖖 Alex AI Integrated Dashboard - Control Panel</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="header">
        <div class="header-content">
            <h1>🖖 Alex AI Integrated Dashboard - Control Panel</h1>
    ...
    <div class="main-content">
        <div class="panel">
            ...
            <div class="control-group">
                ...
                <div class="control-item">
                    ...
                    <input type="text" class="control-input" id="headingInput" value="DEVELOPMENT MODE - THEME STATE FIXED!!!!!">
        ...
        <div class="panel live-preview">
            <div class="live-preview-header">
                ...
                <div class="live-preview-status">
                    <button class="btn secondary">Dashboard Control</button>
                    <div class="status-item">
            ...
            <div class="live-preview-content">
                <div class="preview-title" id="previewTitle">DEVELOPMENT MODE - THEME STATE FIXED!!!!! 🚀</div>
                ...
                <div style="margin: 20px 0; padding: 15px; background: rgba(0, 255, 136, 0.1); border-radius: 6px; border: 1px solid rgba(0, 255, 136, 0.3);">
                    <h4 style="color: #00ff88; margin-bottom: 10px;">🖖 Real-Time Control</h4>
                    <p style="color: #2c3e50; font-size: 13px; margin-bottom: 5px;">Connected to Dashboard</p>
        ...
        <div class="panel">
            ...
            <div class="log-section">
                ...
                <div id="logEntries">
                    <div class="log-entry info">Dashboard initialized and ready for real-time control.</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function storeInSupabase(type, value) {
            // Simulate Supabase storage via N8N middleware
            console.log(`Storing in Supabase via N8N: ${type} = ${value}`);
            
            // In a real implementation, this would call the N8N middleware
            // which would then store the data in Supabase
...
<body>
    ...
    <div class="container">
        ...
        <div class="main-content">
            <div class="header">
                <h1 id="pageTitle">${this.config.heading || this.config.title}</h1>
      ...
      try {
        this.server = http.createServer((req, res) => {
          ...
          if (url.pathname === '/') {
            ...
            res.end(JSON.stringify({
              status: 'healthy',
              timestamp: new Date().toISOString(),
              serverType: 'dashboard-frontend-integration',
            ...
            req.on('end', () => {
              try {
                ...
                console.log(`📦 Storing in Supabase via N8N middleware: ${data.type} = ${data.value}`);
                
                // In a real implementation, this would call N8N middleware
                // which would then store the data in Supabase
                
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({
                  success: true,
                  message: 'Content stored in Supabase via N8N middleware',
                  data: data,
                  timestamp: new Date().toISOString()
                }));
              } catch (error) {
        ...
        this.io.on('connection', (socket) => {
          ...
          socket.emit('dashboard-update', {
            message: 'Connected to Dashboard-Frontend Integration System',
            timestamp: new Date().toISOString()
          });
        ...
        this.server.listen(this.port, () => {
          this.isRunning = true;
          console.log(`🖖 Alex AI Dashboard-Frontend Integration Server running on http://localhost:${this.port}`);
          ...
          console.log(`✅ Dashboard-Frontend integration system ready`);
  ...
  handleDashboardCommand(socket, data) {
    try {
      ...
      socket.emit('dashboard-update', {
        message: `Command processed: ${type} - ${target}`,
        timestamp: new Date().toISOString()
      });
  ...
  async stop() {
    return new Promise((resolve) => {
      if (this.server && this.isRunning) {
        this.server.close(() => {
          this.isRunning = false;
          console.log(`🛑 Alex AI Dashboard-Frontend Integration Server stopped on port ${this.port}`);
          resolve();
        });
      } else {
        resolve();
...
if (require.main === module) {
  const server = new DashboardFrontendIntegrationServer();
  ...
  // Graceful shutdown
  process.on('SIGTERM', () => {
    console.log('SIGTERM signal received. Shutting down dashboard-frontend integration server...');
    server.stop().then(() => process.exit(0));
  });

  process.on('SIGINT', () => {
    console.log('SIGINT signal received. Shutting down dashboard-frontend integration server...');
    server.stop().then(() => process.exit(0));
  });
}

module.exports = DashboardFrontendIntegrationServer;
