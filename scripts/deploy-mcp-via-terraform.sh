...
if [ -z "$ROUTE53_ZONE_ID" ]; then
    ...
    if [ -f "scripts/get-terraform-route53-zone.sh" ]; then
        ROUTE53_ZONE_ID=$(bash scripts/get-terraform-route53-zone.sh 2>/dev/null || echo "")
    fi
fi
...
if [ -z "$EC2_IP" ] || [ "$EC2_IP" == "None" ]; then
    echo "⚠️  Could not get EC2 IP from tags, trying AWS CLI..."
    EC2_IP=$(aws ec2 describe-instances \
...
if [ -z "$EC2_IP" ] || [ "$EC2_IP" == "None" ]; then
    echo "⚠️  Could not get EC2 IP from AWS, trying hostname resolution..."
    EC2_IP=$(getent hosts $EC2_HOST 2>/dev/null | awk '{print $1}' | head -1 || echo "")
fi
