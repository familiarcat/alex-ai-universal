...
class AntiHallucinationCLI {
    ...
    async testSystem(options) {
        try {
            ...
            if (options.scenarios) {
                ...
                testPrompts = testPrompts.filter(prompt => scenarios.some(scenario => prompt.toLowerCase().includes(scenario)));
            }
     ...
     */
    async correctHallucination(options) {
        try {
            if (!this.system) {
                throw new Error('Anti-hallucination system not initialized');
            }
    ...
    async showStatus(options) {
        try {
            ...
            if (options.verbose) {
                console.log('\n📊 Detailed Metrics:');
                console.log(core_1.AntiHallucinationUtils.formatMetrics(status.metrics));
            }
        }
        catch (error) {
    ...
    async manageConfig(options) {
        try {
            ...
            if (options.validate) {
                const errors = core_1.AntiHallucinationUtils.validateConfig(this.config);
                if (errors.length === 0) {
...
//# sourceMappingURL=anti-hallucination-cli.js.map