...
// Mock classes for now - these would be imported from @alex-ai/core
class MinimalAlexAI {
  async initialize() {
    console.log('Alex AI initialized');
  }
...
class CrewSelfDiscoveryCLI {
  initializeCommands(program) {
    program
      .command('crew-discovery <action>')
      .description('Crew self-discovery system')
      .action((action) => {
        console.log(`Crew discovery: ${action}`);
      });
  }
}

class N8NWorkflowCLI {
  initializeCommands(program) {
    program
      .command('n8n-workflows <action>')
      .description('N8N workflow management')
      .action((action) => {
        console.log(`N8N workflows: ${action}`);
      });
  }
}

class ScenarioAnalysisCLI {
  initializeCommands(program) {
    program
      .command('scenario-analysis <action>')
      .description('Scenario analysis system')
      .action((action) => {
        console.log(`Scenario analysis: ${action}`);
      });
  }
}
...
class CrewConsciousnessCLI {
  initializeCommands(program) {
    program
      .command('crew-consciousness <action>')
      .description('Crew consciousness system')
      .action((action) => {
        console.log(`Crew consciousness: ${action}`);
      });
  }
}

class AntiHallucinationCLI {
  setupCommands(program) {
    program
      .command('anti-hallucination <action>')
      .description('Anti-hallucination system')
      .action((action) => {
        console.log(`Anti-hallucination: ${action}`);
      });
  }
}

class N8NIntegrationCLI {
  getProgram() {
    const program = new commander.Command();
    program
      .command('n8n-integration <action>')
      .description('N8N integration system')
      .action((action) => {
        console.log(`N8N integration: ${action}`);
      });
    return program;
  }
}
...
class DebuggingCLI {
  getProgram() {
    const program = new commander.Command();
    program
      .command('debugging <action>')
      .description('Debugging system')
      .action((action) => {
        console.log(`Debugging: ${action}`);
      });
    return program;
  }
}
...
function isN8NIntegrationRequest(message) {
  const n8nKeywords = [
...
function isDebuggingRequest(message) {
...
async function handleN8NIntegrationRequest(message) {
...
async function handleDebuggingRequest(message) {
