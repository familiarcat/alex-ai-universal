...
import { CrewSelfDiscoverySystem, CrewMemberFeature, SelfDiscoveryReport } from './crew-self-discovery';
import * as commander from 'commander';

export class CrewSelfDiscoveryCLI {
  private discoverySystem: CrewSelfDiscoverySystem;

  constructor() {
    this.discoverySystem = new CrewSelfDiscoverySystem();
  }
  ...
  initializeCommands(program: commander.Command): void {
    ...
    discoveryCommand
      ...
      .action(async (crewMember: string, name: string, description: string, category: string, options: any) => {
        try {
          const feature: Omit<CrewMemberFeature, 'addedBy' | 'addedAt' | 'status' | 'introspection'> = {
            id: `feature-${Date.now()}`,
            name,
            description,
    ...
    discoveryCommand
      ...
      .action(async (crewMember: string, sessionId: string) => {
        try {
          ...
          console.log(`\nChallenges:`);
          introspection.challenges.forEach((challenge: string, index: number) => {
            console.log(`${index + 1}. ${challenge}`);
          });
          console.log(`\nInsights:`);
          introspection.insights.forEach((insight: string, index: number) => {
   ...
   */
  private displayFullReport(report: SelfDiscoveryReport): void {
  ...
  private async runCompleteDemo(): Promise<void> {
    ...
    const sessions: { crewMember: string; sessionId: string }[] = [];
   ...
   */
  private getDemoFeaturesForCrewMember(crewMember: string): Omit<CrewMemberFeature, 'addedBy' | 'addedAt' | 'status' | 'introspection'>[] {
    const featureSets: Record<string, Omit<CrewMemberFeature, 'addedBy' | 'addedAt' | 'status' | 'introspection'>[]> = {
      'Captain Picard': [
        {
