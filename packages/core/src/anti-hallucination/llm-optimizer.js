...
class LLMOptimizer {
     ...
     */
    async selectOptimalLLM(crewMember, prompt) {
    ...
    detectPromptType(prompt) {
        ...
        if (creativeKeywords.some(keyword => lowerPrompt.includes(keyword)))
            return 'creative';
        if (analyticalKeywords.some(keyword => lowerPrompt.includes(keyword)))
            return 'analytical';
        if (empathicKeywords.some(keyword => lowerPrompt.includes(keyword)))
            return 'empathic';
        if (strategicKeywords.some(keyword => lowerPrompt.includes(keyword)))
            return 'strategic';
     ...
     */
    async queryOpenRouter(context, skills) {
        try {
     ...
     */
    selectOptimalModel(models, context, skills) {
        const modelScores = new Map();
        for (const model of models) {
            let score = 0;
            // Score based on context type
            if (context.type === 'technical' && this.isTechnicalModel(model.id))
                score += 3;
            if (context.type === 'empathic' && this.isEmpathicModel(model.id))
                score += 3;
            if (context.type === 'creative' && this.isCreativeModel(model.id))
                score += 3;
            if (context.type === 'analytical' && this.isAnalyticalModel(model.id))
                score += 3;
            if (context.type === 'strategic' && this.isStrategicModel(model.id))
                score += 3;
            // Score based on complexity
            if (context.complexity === 'high' && model.context_length > 100000)
                score += 2;
            if (context.complexity === 'medium' && model.context_length > 50000)
                score += 1;
            // Score based on skills match
            if (this.skillsMatchModel(model.id, skills))
                score += 2;
            ...
            if (model.id.includes('gpt-4'))
                score += 2;
            if (model.id.includes('claude-3-sonnet'))
                score += 1;
     ...
     */
    skillsMatchModel(modelId, skills) {
     ...
     */
    getFallbackLLM(context, skills) {
        if (context.type === 'technical')
            return 'openai/gpt-4-turbo';
        if (context.type === 'empathic')
            return 'anthropic/claude-3-sonnet';
        if (context.type === 'creative')
            return 'anthropic/claude-3-sonnet';
        if (context.type === 'analytical')
            return 'openai/gpt-4-turbo';
     ...
     */
    calculateConfidence(context, llm, skills) {
     ...
     */
    generateReasoning(llm, context, skills) {
        const reasons = [];
        if (context.type === 'technical') {
        ...
        const matchedSkills = skills.filter(skill => this.llmCapabilities.get(llm)?.includes(skill));
     ...
     */
    getCrewMemberSkills(crewMember) {
...
//# sourceMappingURL=llm-optimizer.js.map