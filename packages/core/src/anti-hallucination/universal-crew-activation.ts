...
export class UniversalCrewActivation {
   ...
   */
  async activateAllCrewMembers(prompt: string): Promise<CrewActivationResult> {
    ...
    try {
      ...
      if (this.config.parallelProcessing) {
        ...
        const crewPromises = this.crewMembers.map(member => 
          this.processCrewMember(member, prompt)
        );
   ...
   */
  private async processCrewMember(crewMember: string, prompt: string): Promise<CrewPerspective> {
    const startTime = Date.now();
   ...
   */
  private async processWithOptimizedLLM(
    crewMember: string,
    prompt: string,
    ...
    const processingPromise = this.callLLMWithRetry(
      optimization.optimalLLM,
      this.buildCrewMemberPrompt(crewMember, prompt),
      optimization
    );
   ...
   * Generate crew member specific response
   */
  private generateCrewMemberResponse(crewMember: string, prompt: string, llmId: string): string {
   ...
   */
  private buildCrewMemberPrompt(crewMember: string, originalPrompt: string): string {
...
Your Response:`;
   ...
   */
  private getCrewMemberPersona(crewMember: string): { description: string; expertise: string[] } {
   ...
   */
  private async generateFallbackResponse(crewMember: string, prompt: string): Promise<CrewPerspective> {
