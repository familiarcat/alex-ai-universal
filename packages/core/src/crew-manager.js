...
class AlexAICrewManager {
     ...
     * Test connection to N8N Federation Crew
     */
    async testFederationConnection() {
        try {
     ...
     */
    updateCrewMemberStatus(id, status) {
    ...
    getInteractionsForMember(memberId) {
        return this.interactions.filter(interaction => interaction.from === memberId || interaction.to === memberId);
    }
    /**
     * Coordinate with N8N Federation Crew
     */
    async coordinateWithFederation(action, data) {
        try {
            const credentials = await this.credentialsManager.getN8NCredentials();
            const response = await axios_1.default.post(`${credentials.baseUrl}/webhook/federation-mission`, {
                action: action,
     ...
     */
    async assignTask(memberId, task) {
     ...
     * Complete task
     */
    async completeTask(interactionId, result) {
...
exports.AlexAICrewManager = AlexAICrewManager;
//# sourceMappingURL=crew-manager.js.map