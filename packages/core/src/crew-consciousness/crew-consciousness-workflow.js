...
exports.CrewConsciousnessWorkflow = void 0;
const crew_manager_1 = require("../crew-manager");
const crew_workflow_updater_1 = require("../n8n/crew-workflow-updater");
class CrewConsciousnessWorkflow {
    constructor(n8nCreds, supabaseConf) {
        this.activeSessions = new Map();
        this.crewLearningHistory = new Map();
        this.crewManager = new crew_manager_1.AlexAICrewManager();
        this.workflowUpdater = new crew_workflow_updater_1.CrewWorkflowUpdater(n8nCreds, supabaseConf);
        this.initializeCrewLearningHistory();
    }
    initializeCrewLearningHistory() {
        this.crewManager.getCrewMembers().forEach((member) => {
     ...
     */
    async initializeSession(projectRequest) {
        const sessionId = `crew-consciousness-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
        const session = {
            sessionId,
            projectRequest,
            crewMembers: this.crewManager.getCrewMembers().map((member) => member.name.toLowerCase().replace(/\s+/g, '-')),
            startTime: new Date(),
     ...
     */
    async processCrewMemberAnalysis(sessionId, crewMemberId) {
        ...
        const analysis = await this.generateCrewMemberAnalysis(crewMember, session.projectRequest);
        // Store in session
     ...
     */
    async generateCrewMemberAnalysis(crewMember, projectRequest) {
        const analysis = {
     ...
     */
    async storeCrewAnalysisInRAG(crewMemberId, analysis, sessionId) {
     ...
     */
    async generateCollectiveInsights(sessionId) {
        const session = this.activeSessions.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
     ...
     */
    async updateN8NWorkflowsWithSession(session) {
     ...
     */
    synthesizeStrategicOverview(analyses, projectRequest) {
    ...
    synthesizeTechnicalSummary(analyses, projectRequest) {
    ...
    synthesizeBusinessValue(analyses, projectRequest) {
    ...
    synthesizeRiskAssessment(analyses, projectRequest) {
    ...
    synthesizeImplementationPlan(analyses, projectRequest) {
    ...
    synthesizeSuccessMetrics(analyses, projectRequest) {
        return [
    ...
    synthesizeCrewSynergy(analyses) {
    ...
    synthesizeSystemEvolution(analyses) {
...
exports.CrewConsciousnessWorkflow = CrewConsciousnessWorkflow;
//# sourceMappingURL=crew-consciousness-workflow.js.map