...
PROJECT INSIGHTS:
${learning.projectInsights.map((insight: string, index: number) => `  ${index + 1}. ${insight}`).join('\n')}

${characterProfile.name} ${characterProfile.technical_action}, ${characterProfile.technical_motivation}

TECHNICAL LEARNINGS:
${learning.technicalLearnings.map((learning: string, index: number) => `  ${index + 1}. ${learning}`).join('\n')}

${characterProfile.name} ${characterProfile.client_action}, ${characterProfile.client_motivation}

CLIENT UNDERSTANDING:
${learning.clientUnderstanding.map((understanding: string, index: number) => `  ${index + 1}. ${understanding}`).join('\n')}

${characterProfile.name} ${characterProfile.collaboration_action}, ${characterProfile.collaboration_motivation}

COLLABORATION INSIGHTS:
${learning.collaborationInsights.map((insight: string, index: number) => `  ${index + 1}. ${insight}`).join('\n')}
