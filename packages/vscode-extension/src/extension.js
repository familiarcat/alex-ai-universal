"use strict";
/**
 * Alex AI Universal VSCode Extension
 *
 * Provides VSCode integration for Alex AI with Star Trek crew-based AI assistance
 * ZERO ARTIFACT GUARANTEE - No files created in user projects
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.activate = activate;
exports.deactivate = deactivate;
var vscode = require("vscode");
var universal_extension_1 = require("@alex-ai/universal-extension");
// Create the universal extension using VS Code adapter
var _a = (0, universal_extension_1.createVSCodeExtension)(vscode), core = _a.core, commands = _a.commands;
function activate(context) {
    var _this = this;
    console.log('🚀 Alex AI Universal extension is now active!');
    // Initialize the universal core
    core.initialize().then(function () {
        console.log('✅ Alex AI Universal core initialized with zero-artifact guarantee');
    }).catch(console.error);
    // Register Alex AI Engage command
    var engageCommand = vscode.commands.registerCommand('alex-ai.engage', function () { return __awaiter(_this, void 0, void 0, function () {
        var userInput, response, doc, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, vscode.window.showInputBox({
                        prompt: 'Engage Alex AI',
                        placeHolder: 'Ask the crew for assistance...'
                    })];
                case 1:
                    userInput = _a.sent();
                    if (!userInput) return [3 /*break*/, 9];
                    _a.label = 2;
                case 2:
                    _a.trys.push([2, 8, , 9]);
                    return [4 /*yield*/, core.processMessage(userInput)];
                case 3:
                    response = _a.sent();
                    if (!response.success) return [3 /*break*/, 6];
                    return [4 /*yield*/, vscode.workspace.openTextDocument({
                            content: response.coordinatedResponse,
                            language: 'markdown'
                        })];
                case 4:
                    doc = _a.sent();
                    return [4 /*yield*/, vscode.window.showTextDocument(doc)];
                case 5:
                    _a.sent();
                    return [3 /*break*/, 7];
                case 6:
                    vscode.window.showErrorMessage("Alex AI Error: ".concat(response.message));
                    _a.label = 7;
                case 7: return [3 /*break*/, 9];
                case 8:
                    error_1 = _a.sent();
                    vscode.window.showErrorMessage("Alex AI Error: ".concat(error_1.message));
                    return [3 /*break*/, 9];
                case 9: return [2 /*return*/];
            }
        });
    }); });
    // Register Alex AI Status command
    var statusCommand = vscode.commands.registerCommand('alex-ai.status', function () { return __awaiter(_this, void 0, void 0, function () {
        var response, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, core.processMessage('Show system status')];
                case 1:
                    response = _a.sent();
                    vscode.window.showInformationMessage("Alex AI Status: ".concat(response.coordinatedResponse));
                    return [3 /*break*/, 3];
                case 2:
                    error_2 = _a.sent();
                    vscode.window.showErrorMessage("Status check failed: ".concat(error_2.message));
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    }); });
    context.subscriptions.push(engageCommand, statusCommand);
    // Register Alex AI status bar item
    var statusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 100);
    statusBarItem.text = '$(star) Alex AI';
    statusBarItem.tooltip = 'Alex AI Universal - Star Trek Crew-based AI Assistant (Zero Artifacts)';
    statusBarItem.command = 'alex-ai.engage';
    statusBarItem.show();
    context.subscriptions.push(statusBarItem);
    console.log('✅ Alex AI Universal extension initialized with zero-artifact guarantee');
}
function deactivate() {
    console.log('🛑 Alex AI Universal extension is now deactivated');
}
