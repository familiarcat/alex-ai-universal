#!/bin/bash
# Build Simple Sync Dashboard - Without Next.js API Routes

set -e

echo "🚀 Building Simple Sync Dashboard"
echo "==============================="
echo ""
...
echo ""
echo "🔧 Phase 2: Build Static Version"
echo "================================"

# Build static version (without API routes)
echo "🔨 Building static version..."
export NODE_ENV=production
npm run build

# Copy to both build directories
echo "📁 Copying to build directories..."
cp -r ${BUILD_DIR}/* ${LOCAL_BUILD_DIR}/
cp -r ${BUILD_DIR}/* ${DEPLOYED_BUILD_DIR}/

echo "✅ Static version built and copied"

echo ""
echo "🔧 Phase 3: Create Sync Servers"
echo "=============================="

# Create local server with sync API
...
# Create deployed server with sync API
