...
const defaultProjects = {
  ...
  temporal: {
    ...
    theme: 'offworld',
    projectType: 'creative' as const,
    updatedAt: Date.now()
  }
};

async function seedProjectToSupabase(projectId: string, content: any) {
  const N8N_URL = process.env.N8N_URL || 'https://n8n.pbradygeorgen.com';
  
  console.log(`📤 Syncing ${projectId} to Supabase via n8n...`);
  
  try {
    const response = await fetch(`${N8N_URL}/webhook/content-sync`, {
      method: 'POST',
      headers: {
...
async function seedAllProjects() {
  ...
  if (successful === 4) {
    ...
    console.log(`\nWebhook URL: ${process.env.N8N_URL || 'https://n8n.pbradygeorgen.com'}/webhook/content-sync`);
  }
}

// Run if executed directly
if (require.main === module) {
  seedAllProjects().catch(console.error);
}

export { seedAllProjects, seedProjectToSupabase, defaultProjects };
