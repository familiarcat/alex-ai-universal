...
"[project]/dashboard/lib/mock-data-system.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class MockDataSystem {
   ...
   */ generateLearningMetrics(config = {
        ...
        for(let i = count - 1; i >= 0; i--){
            ...
            sessions.push({
                ...
                complexity_level: Math.floor(Math.random() * 5) + 3,
   ...
   */ generateCrewStats(config = {
        ...
        const data = {
            stats,
            totalMemories: stats.reduce((sum, s)=>sum + s.memories, 0)
        };
        this.mockDataCache.set(cacheKey, data);
        return data;
    }
   ...
   */ generateProjectRecommendations(config = {
        componentName: 'RAGProjectRecommendations'
    }) {
   ...
   */ generateSecurityData(config = {
        componentName: 'SecurityAssessmentDashboard'
    }) {
   ...
   */ generateCostData(config = {
        componentName: 'CostOptimizationMonitor'
    }) {
   ...
   */ generateUXData(config = {
        componentName: 'UserExperienceAnalytics'
    }) {
            ...
            feedback: [
                {
                    id: 'fb-1',
                    rating: 5,
                    comment: 'Mock positive feedback',
                    date: new Date().toISOString()
                },
                {
                    id: 'fb-2',
                    rating: 4,
                    comment: 'Mock constructive feedback',
                    date: new Date().toISOString()
                }
            ]
        };
   ...
   */ shouldUseMockData(componentName, hasLiveData) {
        // Use mock data if:
        ...
        const useMock = __TURBOPACK__imported__module__$5b$project$5d2f$dashboard$2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].env.NEXT_PUBLIC_USE_MOCK_DATA === 'true';
        const allowlist = [
   ...
   */ getMockData(componentName, dataType) {
        switch(componentName){
            ...
            case 'CrewMemoryVisualization':
                return this.generateCrewStats({
                    componentName
                });
            case 'RAGProjectRecommendations':
                return this.generateProjectRecommendations({
                    componentName
                });
            ...
            case 'CostOptimizationMonitor':
                return this.generateCostData({
                    componentName
                });
            case 'UserExperienceAnalytics':
                return this.generateUXData({
                    componentName
                });
            default:
                return {
                    data: [],
                    error: `No mock data generator for ${componentName}`
                };
        }
    }
    // Helper methods
...
const mockDataSystem = MockDataSystem.getInstance();
...
//# sourceMappingURL=dashboard_lib_mock-data-system_ts_72f5ca69._.js.map