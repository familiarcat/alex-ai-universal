...
export default function WizardInline({ projectId, onApply }: WizardInlineProps) {
  ...
  return (
    <div style={{ border: 'var(--border)', padding: 16, borderRadius: 12, background: 'var(--card)' }}>
      ...
      {step === 2 && (
        <div>
          <div style={{ marginBottom: 8 }}>Choose a theme</div>
          <select value={theme} onChange={(e) => setTheme(e.target.value)} style={{ padding: 10, background: 'var(--card-alt)', color: 'var(--text)', border: 'var(--border)', borderRadius: 8 }}>
